module potvars
use allvars
character(len=100)  :: bond_pair_file
integer,allocatable :: bond_pair(:,:)
real(8)             :: k_bond, k_ang,bond_eq,ang_eq
contains 
subroutine initialize_potvars()
    use allvars, only:tot_bond_angles
    use constants, only : length_au_inv
    implicit none 
    integer:: i,j
    open(unit=211, file = "pot.param", status = "old")
    read(211,*)bond_pair_file
    read(211,*)bond_eq
    read(211,*)ang_eq
    read(211,*)k_bond
    read(211,*)k_ang
    allocate(bond_pair(0:tot_bond_angles-1,0:3))
    bond_pair = 0
    open(unit=44,  file = ""//trim(adjustl(bond_pair_file))//"",status="old")
    do i = 0,tot_bond_angles-1
       read(44,*)(bond_pair(i,j),j = 0,2)
    enddo
    bond_eq = bond_eq * length_au_inv 
end subroutine
!************************************************************************************************!
!************************************************************************************************!
